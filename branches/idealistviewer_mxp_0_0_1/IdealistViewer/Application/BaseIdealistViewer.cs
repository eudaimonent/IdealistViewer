...
namespace IdealistViewer
{
    public class BaseIdealistViewer : conscmd_callback
    {
        ...
        private IDictionary<ulong, IList<SceneNode>> terrainSceneNodes = new Dictionary<ulong, IList<SceneNode>>();
            ...
            while (running)
            {
                ...
                */
                ...
                if (m_dirtyTerrain.Count>0)
                {
                    ...
                    if (!terrainSceneNodes.ContainsKey(currentSim.Handle))
                    {
                        terrainSceneNodes.Add(currentSim.Handle, new List<SceneNode>());
                    }

                    IList<SceneNode> sceneNodes=terrainSceneNodes[currentSim.Handle];
                    ...
                    for (y = 0; y < 256; y += yStep)
                    {
                        ...
                        for (x = 0; x < 256; x += xStep)
                        {
                            ...
                            float[,] heights = subHeightField(x, x1, y, y1);
                            ...
                            sampleHFNode.SetMaterialTexture(0, GreenGrassTexture);
                ...
                foreach (string str in interpolationTargets.Keys)
                {
                    ...
                    try
                    {
                        ...
                        */
                        ...
                        Vector3 distance = obj.position - obj.prim.Position;

                        obj.prim.Position = obj.prim.Position + distance * 0.1f;
                        obj.node.Position = new Vector3D(obj.prim.Position.X, obj.prim.Position.Z, obj.prim.Position.Y);
                    }
                    catch (AccessViolationException)
                    {
                        //if (removestr == null)
        ...
        private void UpdateFriendsList()
        {
            ...
            foreach(UUID friend in avatarConnection.Friends.Keys )
            {
                ...
                {
                    friendsList.AddItem(avatarConnection.Friends[friend].Name + statusString);
                }
            }
        }
            ...
            lock (Entities)
            {
                if (Entities.ContainsKey(regionHandle.ToString() + update.LocalID.ToString()))
                {
                    ...
                    if (obj.prim is Avatar)
                    {
                        ...
                        obj.position = update.Position + new Vector3(0.0f, 0.0f, (float)(-obj.node.BoundingBox.MinEdge.Y*obj.node.Scale.Y));
                    }
                    else
                    {
                        obj.updateFullYN = false;
                                       ...
                                       ushort timeDilation)
        {
            ...
            lock (Entities)
            {
                ...
                if (Entities.ContainsKey(regionHandle.ToString() + avatar.LocalID.ToString()))
                {
                    ...
                    avob = existingob;
                    avob.position = new Vector3(avatar.Position.X, avatar.Position.Y, avatar.Position.Z);
                    if (!interpolationTargets.ContainsKey(regionHandle.ToString() + avatar.LocalID.ToString()))
                    {
                        interpolationTargets.Add(regionHandle.ToString() + avatar.LocalID.ToString(), avob);
                    }
                }
                else
                {
